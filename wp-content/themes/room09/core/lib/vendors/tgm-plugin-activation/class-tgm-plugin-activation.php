			...
			if ( ! get_user_meta( get_current_user_id(), 'tgmpa_dismissed_notice', true ) ) {
				...
				if ( ! empty( $message ) ) {
					...
					foreach ( $message as $type => $plugin_groups ) {
						...
						$rendered .= apply_filters("tgmpa_message_type_" . $type, "<p class='{$type}'>" . sprintf( translate_nooped_plural( $this->strings[$type], $count, $this->domain ), $imploded, $count ) . '</p>'); // All messages now stored
					}
					...
					if ( $action_links )
						$rendered .= '<p class="action_links">' . implode( ' | ', $action_links ) . '</p>';
