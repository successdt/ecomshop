...
class YIT_Slider {     
    ...
    protected function _getSliderTypes() {          
        ...
        // exception of revolution slide
        if ( ! class_exists( 'GlobalsRevSlider' ) && isset( $this->_sliderTypes['revolution-slider'] ) ) unset( $this->_sliderTypes['revolution-slider'] );
    }          
    ...
    public function links_slider()
    {
        ...
        if ( $this->_there_is_link ) {
            $target = ( strpos( $this->_url_slide, site_url() ) === false ) ? ' target="_blank"' : '';
    ...
    public function slider_shortcode( $atts, $content = null ) {  
        ...
        if ( yit_get_option('responsive-enabled') && ( ! isset( $this->responsive_sliders[ $this->get('slider_type') ] ) || ! $this->responsive_sliders[ $this->get('slider_type') ] ) && ! empty( $responsive_mode ) && $responsive_mode != 'none' ) {
    ...
    public function edit_columns( $columns ) {
        $columns['default'] = __( 'Set in Home Page', 'yit' ) . ' <img src="' . esc_url( admin_url( 'images/wpspin_light.gif' ) ) . '" class="ajax-loading default-ajax-loading" alt="" />';
