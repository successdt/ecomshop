...
//         }
        return add_query_arg( $cpt_item, get_permalink( $post_id ) );
    }    
    ...
    public function get_term_link( $args = array() ) {
        ...
        if ( empty( $permalink ) ) { //temporany solution
            return add_query_arg( array( 'post_type' => $post_type, 'p' => $post_id, 'cat' => $cat ), home_url() );
        } else {
            $post = get_post($post_id);                                     
            return add_query_arg( array( 'cat' => $cat ), home_url("{$post->post_type}/{$post->post_name}/") );
        }
    }                 
