...
class YIT_Panel {         
    ...
    /**
     * Base name for the plugin options
     *
     * @since 1.0.0
     * @var string
     */
    public $plugins_name = 'yit_plugins';
    ...
    protected function _init_tree() {
        $this->_tree = apply_filters( 'yit_admin_tree', array(
    		'page' => array(
...
//    			)
	...
	protected function _setOptionName() {
        ...
        $this->plugins_name .= '_' . YIT_THEME_NAME;
    }
	...
	public function add_submenu_page() {
		foreach( $this->_tree['subpages'] as $k=>$subpage ){
			//add the page
			add_submenu_page(
				$subpage['parent_slug'],
    ...
    {                                                                               
        ...
        elseif ( is_shop_installed() && ( is_shop() || is_product_category() || is_product_tag() ) ) $post_id = woocommerce_get_page_id( 'shop' );
    ...
    public function save_options_callback() {
		...
		if( isset( $_POST['yit-subpage'] ) && $_POST['yit-subpage'] == 'backup' ) {
    	   ...
    	   { do_action( 'yit_save_panel' ); return; }
    	...
    	foreach ( $page_options as $tab_path => $options ) {
            foreach ( $options as $option ) {
                ...
                //save the plugins
                if( isset( $_POST['yit-subpage'] ) && $_POST['yit-subpage'] == 'plugins' ) {
                    $yit_plugins[ $option['id'] ] = $value;
                }
            }
        }

        if( !empty($yit_plugins) ) {
            update_option($this->plugins_name, $yit_plugins);
        }
