...
class YIT_Widget_Best_Sellers extends WP_Widget {
	 ...
	 * @return void
	 */
	function YIT_Widget_Best_Sellers() {
		...
		$this->woo_widget_idbase = 'yit_best_sellers';
		$this->woo_widget_name = __( 'YIT Best Sellers', 'yit' );
		...
		if ( $r->have_posts() ) {
			...
			while ( $r->have_posts()) {
				...
				echo '<li>
					...
					<div class="number-position"><p>'. str_pad($number, 2, 0, STR_PAD_LEFT) .'</p></div>
				</li>';
				$number++;
			}
